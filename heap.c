...
void heap_downheap(void** datos, size_t cant, size_t index_elem, cmp_func_t cmp) {
	...
	size_t index_hijo_izq = 2*index_elem + 1;
	size_t index_hijo_der = 2*index_elem + 2;
	size_t index_hijo = cant < index_hijo_der + 1 || cmp(datos[index_hijo_izq], datos[index_hijo_der]) > 0 ? index_hijo_izq : index_hijo_der;
...
bool heap_encolar(heap_t *heap, void* elem) {
	...
	heap->cant++;
	heap_upheap(heap->datos, heap->cant, heap->cant-1, heap->cmp);
	return true;
}
